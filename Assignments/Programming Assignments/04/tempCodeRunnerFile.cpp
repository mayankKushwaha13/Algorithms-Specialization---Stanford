...
double tsp(vector<vector<double>> &dist, int mask, int s, const int &VISITED_ALL)
{
    int n = dist.size();
    ...
    for (int city = 0; city < n; city++)
    {
        if ((mask & (1 << city)) == 0)
        {
            double newAns = dist[s][city] + tsp(dist, mask | (1 << city), city, VISITED_ALL);
            ans = min(ans, newAns);
        }
    }
...
int main()
{
    ...
    double tsp1 = tsp(dist, 1 << 11, 11, VISITED_ALL);
    ...
    VISITED_ALL = (1 << 14) - 1;
    double tsp2 = tsp(dist2, 1 << 0, 0, VISITED_ALL);
    double connect = sqrt(
